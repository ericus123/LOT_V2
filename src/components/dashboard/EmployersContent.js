...
const handleSaveData = (data,rate_nbr) => {
  setData(data);
  setRate(rate_nbr);
};
...
return (
     <>
      <RateModal rate={rate} modal_data={modal_data} rateVisible={rateVisible} showRateModal={showRateModal} hideRateModal={hideRateModal}/>
   ...
   {companies?.map((item,key) => 
       ...
       company-name-col">{item.name}</Col><Col className="rate-col" span={8}><Rate className="rating-stars" allowHalf defaultValue={Number(item.rating)} /></Col> <Col className="rate-text-col cursor_pointer" span={6} onClick={() => {showRateModal(); handleSaveData({name:item.name,icon:item.picture,rating: Number(item.rating)},item.rating);}}>Rate</Col>
